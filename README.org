...
* Start here: installation and required downloads
:PROPERTIES:
:UNNUMBERED: notoc
:END:

See [[file:INSTALLATION.md][INSTALLATION.md]] before running the examples. BuMoChi includes its own
SuperCollider, Python, protocol-test, and Godot example code. XR Animator,
SuperCollider, Godot, and OSCGroups are external projects and are not bundled
with this repository. The installation guide links to their official download
pages and explains which components are required for each pipeline test.

Earlier version: (sc-dance)
(To be reviewed for alpha release of present library)
...
* VMC conversion and OSC recording
:PROPERTIES:
:UNNUMBERED: notoc
:END:

Dependency-free Python bridges for XR Animator, SuperCollider/OscGroups, and
Godot are provided in =./HelperAppsAndExamples/VMC_Converter_Scripts/=. The
scripts handle VMC (Virtual Motion Capture Protocol). See the README there for
the canonical =/BuMoChi/VMC/Packet= format and command-line examples.

=OscRecorder= stores new =.scd= sessions below
=Platform.userAppSupportDir/OSC_Data=. Date and session timestamp subfolders
remain unchanged. On macOS, this normally resolves below the user's
=~/Library/Application Support/SuperCollider= directory.
* Example files
:PROPERTIES:
:UNNUMBERED: notoc
:END:
