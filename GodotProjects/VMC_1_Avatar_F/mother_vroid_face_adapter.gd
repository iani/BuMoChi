class_name MotherVRoidFaceAdapter
extends Node

## Supplements XRFaceModifier3D for VRoid blend-shape names that are not part
## of Godot's Unified Expressions/ARKit name tables.
...
func _ready() -> void:
	...
	if _target_mesh == null or _target_mesh.mesh == null:
		push_warning("MotherVRoidFaceAdapter target is not a valid face mesh")
		set_process(false)
		return
	...
	if _shape_indices.is_empty():
		push_warning("MotherVRoidFaceAdapter found no supported VRoid blend shapes")
