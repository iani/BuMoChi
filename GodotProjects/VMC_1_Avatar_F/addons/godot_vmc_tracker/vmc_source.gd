...
func _on_vmc_ext_blend_val(entry : Array) -> void:
	...
	if blend >= 0:
		_face_blends[blend] = clampf(float(entry[2]), 0.0, 1.0)
		_new_face_blends = true
		return

	# XR-Animator commonly transmits VRM expression/preset names rather than
	# ARKit names. Expand those aliases into the corresponding tracker channels.
	var alias_name := str(entry[1]).strip_edges().to_lower()
	var alias_blends : Array = VMCBody.FACE_BLEND_ALIASES.get(alias_name, [])
	if alias_blends.is_empty():
		return

	var weight := clampf(float(entry[2]), 0.0, 1.0)
	for alias_blend in alias_blends:
		_face_blends[alias_blend] = weight
	_new_face_blends = true
