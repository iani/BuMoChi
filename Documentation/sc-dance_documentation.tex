...
\begin{itemize}
\item `sessionData`: An instance of `SessionData` that holds the loaded motion capture data for the avatar.
...
\begin{itemize}
\item \textbf{Loading Data}: `Avatar` can load MOCAP session data from files, which is then managed by its `sessionData` instance.
