...
この時点ではポートを設定しないでください。完全な起動手順とポート設定については、[BuMoChi フルセットアップ手順](HelpByTopic/BuMoChi%20フルセットアップ手順.md)および [Port Number Setup](HelpByTopic/PortNumberSetup.md)を参照してください。
...
以上でインストールは完了です。小さなユーザーテストを順番に行うには[はじめに](はじめに.md)へ進み、XR-Animator-BuMoChi-Godot パイプライン全体を起動して接続するには[BuMoChi フルセットアップ手順](HelpByTopic/BuMoChi%20フルセットアップ手順.md)を使用してください。
