...
1. Open the [[https://github.com/ButzYung/SystemAnimatorOnline/releases][official XR-Animator releases page]].
...
Do not configure its port yet. The complete startup and port procedure is in [[file:HelpByTopic/Full%20Setup%20Procedure.md][Full Setup Procedure]] and [[file:HelpByTopic/PortNumberSetup.md][Port Number Setup]].
...
If the command is unavailable, install Python 3 from [[https://www.python.org/downloads/][python.org]] or through the package manager used by your operating system. Then open a new terminal and run the version check again.
...
Do not copy =BunrakuOSCEncoder.py= by itself because it imports modules beside it. To run it from any terminal directory, create the optional symbolic-link launcher described in [[file:HelpByTopic/HelperApplications/BunrakuOSCEncoder.md#make-bunrakuoscencoder-available-from-any-terminal-directory][Make BunrakuOSCEncoder available from any terminal directory]].

For its purpose, options, and startup commands, see [[file:HelpByTopic/HelperApplications/BunrakuOSCEncoder.md][BunrakuOSCEncoder]].
...
For installation as a globally accessible command and for the nine required startup arguments, see [[file:HelpByTopic/HelperApplications/OSCGroupsClient.md][OscGroupClient]].

The upstream OSCGroups source project is maintained at [[https://github.com/RossBencina/oscgroups][Ross Bencina's OSCGroups repository]].
...
1. Download the current stable version appropriate for your computer from the [[https://supercollider.github.io/downloads][official SuperCollider downloads page]].
...
:PROPERTIES:
:CUSTOM_ID: bumochi-supercollider-library
:END:
...
As with the encoder, keep the entry point beside its support modules. Do not copy the Python file by itself. To run it from any terminal directory, create the optional symbolic-link launcher described in [[file:HelpByTopic/HelperApplications/BunrakuOSCDecoder.md#make-bunrakuoscdecoder-available-from-any-terminal-directory][Make BunrakuOSCDecoder available from any terminal directory]].

For its purpose, options, and startup commands, see [[file:HelpByTopic/HelperApplications/BunrakuOSCDecoder.md][BunrakuOSCDecoder]].
...
1. Download Godot 4.5 from the [[https://godotengine.org/download/archive/4.5-stable/][official Godot 4.5 archive page]].
...
:PROPERTIES:
:CUSTOM_ID: godot-projects-for-bumochi
:END:
...
For avatar receiver ports and tracker names, see [[file:HelpByTopic/PortNumberSetup.md][Port Number Setup]] and [[file:HelpByTopic/Multi-Avatar%20project%20port%20number%20setting%20in%20godot%20-%20NOTES.md][Multi-Avatar project port number setting in Godot]].

* Verify the installation
:PROPERTIES:
:CUSTOM_ID: verify-the-installation
:END:
...
Installation is then complete. Continue with [[file:Getting%20Started.md][Getting Started]] for a small sequence of user tests, or use [[file:HelpByTopic/Full%20Setup%20Procedure.md][Full Setup Procedure]] to start and connect the complete XR-Animator--BuMoChi--Godot pipeline.
