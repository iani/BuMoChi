...
Do not configure its port yet. The complete startup and port procedure is in [[file:HelpByTopic/Setup.md][Setup]] and [[file:HelpByTopic/PortNumberSetup.md][Port Number Setup]].
...
Installation is then complete. Continue with [[file:Getting%20Started.md][Getting Started]] for a small sequence of user tests, or use [[file:HelpByTopic/Setup.md][Setup]] to start and connect the complete XR-Animator--BuMoChi--Godot pipeline.
