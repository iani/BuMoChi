...
#+begin_quote
[!tip] 完全なマルチユーザー起動手順については、[[file:HelpByTopic/BuMoChi%20フルセットアップ手順.md][BuMoChi フルセットアップ手順]]を参照してください。
#+end_quote
...
* ローカルテスト用パイプラインを準備する
:PROPERTIES:
:CUSTOM_ID: ローカルテスト用パイプラインを準備する
:END:
最初のテストをローカルかつ診断しやすい状態に保つため、この準備では OSCGroups を使用しません。

** 1. Godot を起動する
:PROPERTIES:
:CUSTOM_ID: 1-godot-を起動する
:END:
次のプロジェクトを開いて実行します。
...
:PROPERTIES:
:CUSTOM_ID: 2-bunrakuoscdecoder-を起動する
:END:
...
:PROPERTIES:
:CUSTOM_ID: 3-supercollider-と-bumochi-を起動する
:END:
...
:PROPERTIES:
:CUSTOM_ID: 4-bunrakuoscencoder-を起動する
:END:
