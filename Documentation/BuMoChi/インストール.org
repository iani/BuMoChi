...
この時点ではポートを設定しないでください。完全な起動手順とポート設定については、[[file:HelpByTopic/BuMoChi%20フルセットアップ手順.md][BuMoChi フルセットアップ手順]]および [[file:HelpByTopic/PortNumberSetup.md][Port Number Setup]]を参照してください。
...
以上でインストールは完了です。小さなユーザーテストを順番に行うには[[file:はじめに.md][はじめに]]へ進み、XR-Animator-BuMoChi-Godot パイプライン全体を起動して接続するには[[file:HelpByTopic/BuMoChi%20フルセットアップ手順.md][BuMoChi フルセットアップ手順]]を使用してください。
