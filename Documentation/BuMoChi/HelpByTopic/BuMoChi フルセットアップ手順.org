...
2. [@2] OSCGroups を使って共同作業する場合に限り、セッション主催者から =OscGroupServer= のアドレスと一意のユーザー名を取得します。セッションがデフォルトのグループ =bumochi= とパスワード =bmc123= を使用するのか、別の値を使用するのか確認してください。
...
スクリーンショットと詳しい設定手順については、[[file:PortNumberSetup.md#1-xr-animator-vmc-output-port][XR-Animator の VMC 出力ポート]]および [[file:HelperApplications/BunrakuOSCEncoder.md#configure-xr-animator][XR-Animator の設定]]を参照してください。
...
インストール、オプション、診断手順については、[[file:HelperApplications/BunrakuOSCEncoder.md#how-to-start-bunrakuoscencoder][BunrakuOSCEncoder の起動方法]]および [[file:HelperApplications/BunrakuOSCEncoder.md#test-and-diagnose][BunrakuOSCEncoder のテストと診断]]を参照してください。
...
インストール、オプション、診断については、[[file:HelperApplications/BunrakuOSCDecoder.md#how-to-start-bunrakuoscdecoder][BunrakuOSCDecoder の起動方法]]および [[file:HelperApplications/BunrakuOSCDecoder.md#test-and-diagnose][BunrakuOSCDecoder のテストと診断]]を参照してください。
