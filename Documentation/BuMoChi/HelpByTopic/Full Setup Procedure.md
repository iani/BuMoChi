...
| 2 | `BunrakuOSCEncoder` | `39537` | `57130`, `22244` | Listen for XR-Animator; send identical route-free frames to local Bmc on `57130` and local `OscGroupClient` on `22244`. |
...
2. Obtain the `OscGroupServer` address, user credentials, group name, and group password from the session organiser.

3. Give this workstation a unique OSCGroups username, unique encoder `--source` identity, and a free `LOCAL_TO_REMOTE_PORT`.
