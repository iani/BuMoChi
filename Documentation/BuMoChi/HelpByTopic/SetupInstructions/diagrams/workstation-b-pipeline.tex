...
\begin{tikzpicture}[node distance=16mm and 20mm]
  \node[box,fill=vmcblue] (xr) {XR-Animator B\\camera tracking};
  \node[box,fill=bunrakuorange,right=of xr] (enc) {BunrakuOSCEncoder\\avatar: \texttt{PerformerB}\\source: \texttt{workstation-b-}\\\texttt{xr-animator}};
  \node[box,fill=scyellow,right=30mm of enc] (bmc) {SuperCollider / Bmc\\input: UDP \texttt{57130}\\local + remote sources\\synthesizes full scene};
  \node[box,fill=bunrakuorange,right=40mm of bmc] (dec) {BunrakuOSCDecoder\\input: UDP \texttt{39538}\\reads embedded target port};

  \node[smallbox,fill=networkviolet,below=22mm of bmc] (client) {OscGroupClient B\\input port: \texttt{22244}\\output port: \texttt{57130}\\unique username B};
  \node[smallbox,fill=networkviolet,below=18mm of client] (server) {OscGroupServer\\server port: \texttt{22242}\\shared group};

  \node[smallbox,fill=godotgreen,above right=10mm and 19mm of dec] (godot1) {Godot B: Ishidomaru\\from locally synthesized scene\\VMC input: \texttt{39539}};
  \node[smallbox,fill=godotgreen,below right=10mm and 19mm of dec] (godot2) {Godot B: Mother\\from locally synthesized scene\\VMC input: \texttt{39540}};
  ...
  \draw[networknet] (client) -- node[label,right]{Internet / OSCGroups\\send B; receive A} (server);
  \draw[networkflow] (client.north) --
    node[label,midway,right]{remote \texttt{PerformerA} route-free v1\\UDP \texttt{57130}}
    node[font=\sffamily\scriptsize\bfseries,red,align=center,midway,left]{Receive mocap data\\from remote clients} (bmc.south);
  \draw[flow] (bmc) -- node[label,above]{frame-embedded port nr\\UDP \texttt{39538}} (dec);
  ...
  \path (xr.west |- bmc.north) -- (godot1.east |- bmc.north)
    node[font=\sffamily\bfseries,align=center,midway,above=17mm]
    {Workstation B\\Distributed sources, local synthesis and rendering};
  \node[font=\sffamily\scriptsize,align=center,below=4mm of server] {The server forwards B's network copy to other clients;\\it does not echo \texttt{PerformerB} back to Client B.};
\end{tikzpicture}
\end{document}
