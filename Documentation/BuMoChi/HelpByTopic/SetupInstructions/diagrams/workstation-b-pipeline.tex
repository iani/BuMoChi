...
\begin{tikzpicture}[node distance=16mm and 20mm]
  ...
  \draw[flow] (enc.south) |- node[label,pos=.65,above]{identical route-free v1 network copy\\UDP \texttt{22244}} (client.west);
  \draw[net] (client) -- node[label,right]{Internet / OSCGroups\\send B; receive A} (server);
  \draw[flow] (client.east) -| node[label,pos=.72,below]{remote \texttt{PerformerA} route-free v1\\UDP \texttt{57130}} (bmc.south);
