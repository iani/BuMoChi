...
\begin{tikzpicture}[node distance=16mm and 20mm]
  ...
  \node[smallbox,fill=networkviolet,below=22mm of bmc] (client) {OscGroupClient B\\localTx: \texttt{22244}\\localRx: \texttt{57130}\\unique username B};
  ...
  \draw[flow] (enc.south) -- (enc.south |- client.west)
    -- node[label,midway,above]{identical route-free v1 network copy\\UDP \texttt{22244}} (client.west);
  \draw[net] (client) -- node[label,right]{Internet / OSCGroups\\send B; receive A} (server);
  \draw[flow] (client.north) -- node[label,midway,right]{remote \texttt{PerformerA} route-free v1\\UDP \texttt{57130}} (bmc.south);
  ...
  \node[font=\sffamily\bfseries,above=12mm of bmc] {Workstation B -- distributed sources, local synthesis and rendering};
