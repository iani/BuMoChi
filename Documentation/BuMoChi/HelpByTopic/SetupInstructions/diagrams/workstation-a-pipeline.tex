...
\begin{tikzpicture}[node distance=16mm and 20mm]
  ...
  \node[box,fill=scyellow,right=35mm of enc] (bmc) {SuperCollider / Bmc\\input: UDP \texttt{57130}\\local: \texttt{PerformerA}\\remote: \texttt{PerformerB}};
  \node[box,fill=bunrakuorange,right=of bmc] (dec) {BunrakuOSCDecoder\\input: UDP \texttt{39538}\\reads embedded target port};

  \node[smallbox,fill=networkviolet,below=22mm of enc] (client) {OscGroupClient A\\localTx: \texttt{22244}\\localRx: \texttt{57130}\\unique username A};
  ...
  \draw[net] (client) -- node[label,right]{Internet / OSCGroups\\send A; receive B} (server);
  \draw[flow] (client.east) -| node[label,pos=.72,below]{remote \texttt{PerformerB}\\UDP \texttt{57130}} (bmc.south);
  ...
  \node[font=\sffamily\bfseries,above=12mm of bmc] {Workstation A -- local pipeline and remote reception};
