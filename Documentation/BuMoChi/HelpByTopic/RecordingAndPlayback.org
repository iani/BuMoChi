...
A clip is always the complete recorded data. Supplying =startFrame=,
=endFrame=, =rate=, =loop=, bone selection, or playback targets changes only
the manner of playback; it does not create or alter a clip. A named reusable
collection of those choices is a /preset/.
...
6. Optional but recommended: make the live and replayed motion visible in
   ...
   terminal in =PipelineApplications= and start the decoder:
...
The active implementation is [[file:../../../Classes/Animation/OscRecorder.sc][Classes/Animation/OscRecorder.sc]]. The older sc-dance =SessionData= and =OscFile= readers have been archived under =Deprecated/sc-dance-v1=; current BuMoChi does not depend on them. =Bmc.loadClipScd(path)= loads one complete =.scd= clip file. Importing an entire directory of numbered =OscRecorder= chunks as one logical Bmc Clip is not currently implemented.
...
1. Make sure the Python decoder and Godot are running as described in the
   first section. The selected Bmc avatar must still target Godot port
   =39539=; Bmc forwards it to decoder port =39538= automatically:
...
5. Optional transport values: playback =rate=, =loop= state, and the =in= position
   in seconds.
...
An optional =path= may identify a =.bmc= file outside the active AnimationClips
directory:
...
inside one top-level scene data object:
...
Although a Motion name can equal its Clip name, storing the Clip reference explicitly is useful: a dramaturgical cue such as =\motherEntrance= can refer to the complete recording named =\mother_take_01=. The avatar name is required. The host is stored as
well as the port so that the same scene format also works across computers.
...
The scene name becomes the filename. The =.scd= file contains readable
sclang data and may be inspected or edited as text. A custom destination may
be supplied as the fourth argument to =Bmc.saveScene=. Because loading an
=.scd= file asks sclang to interpret its contents, load scene files only
...
=Bmc.loadScene= also accepts an absolute =.scd= path. =Bmc.applyScene=
...
=Bmc.play= and =Bmc.playClip= use the =\default= player unless a =playerName=
is supplied. Named players can run simultaneously and are independently
accessible through =Bmc.player(name)=. =Bmc.playSceneMotion= currently
continues to use the default player.

See [[file:Avatar_Port_Numbers.md][Avatar Port Numbers]] for the complete multi-avatar OSC routing pipeline and terminal commands.
