...
The relevant implementation is [[file:../../../Classes/Animation/OscRecorder.sc][Classes/Animation/OscRecorder.sc]]. Its companion =SessionData= and =OscFile= classes read the sequence of recorder files. By contrast, =Bmc.loadClipScd(path)= loads one complete =.scd= clip file. Loading an entire directory of numbered =OscRecorder= chunks as one Bmc clip is not currently part of =Bmc.loadClipScd=.
