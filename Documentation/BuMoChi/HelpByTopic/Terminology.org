...
A clip is time-indexed mocap data: an ordered collection of frames and their relative playback times.
...
A figure is a logical full-body motion state assembled from one or more sources. It exists between raw motion sources and the rendered avatar.
...
For example, the clip key =\motherEntrance= may refer to the saved clip =\take1=. The clip key describes that clip's function within this session, while the saved clip keeps its original name and recorded data unchanged.
...
Place the cursor in the Mermaid block and evaluate it with =C-c C-c=. After =ob-mermaid= and =mmdc= are configured, Org generates the SVG and displays it inline. If necessary, run =M-x org-redisplay-inline-images=.
...
The current =BmcAvatar= implementation still performs some figure-like pose completion and wire composition. This is an implementation stage, not the final conceptual definition of an avatar.
