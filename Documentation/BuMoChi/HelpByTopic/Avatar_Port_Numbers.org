...
The current =BmcScene= implementation accepts an optional shared decoder setting while retaining its configured Motions that refer to separate, complete Clips:
...
The fuller =motions= and =figures= scene format is specified in [[file:SceneDataObjectsAndTerminology.org][Scene Data Objects and Terminology]] but is not yet implemented by =BmcScene=.
