...
Freezing a player leaves its last cached pose active. Artistically, this can hold one body region while the rest of the avatar continues moving. Stopping removes the player's cache and reveals the sources underneath. Muting also removes the cache, but unlike stopping it lets the clip's playback clock continue advancing invisibly; unmuting lets its next frame return to the top of the stack.
...
Positional modification of the completed avatar frame is now implemented by **BmcPositionModifier**. It can replace or add to selected `Hips` translation axes using a fixed vector or a Function sampled at compositor time. Rotational offsets, motion-range scaling, noise-specific rules, mirroring, constraints, and weighted blending remain design targets. Those future rule objects must identify their inputs, selected bones, reference frame, parameters, and evaluation order, and use the correct mathematics for translations, quaternion rotations, and blends.
...
The first such rule is **BmcPositionModifier**, which transforms the completed frame's `Hips` translation. Other modifying-frame rules still require explicit objects because quaternion rotations, scaling, and weighted blends require different mathematics.
...
Freezing leaves the player's last cache active, so its pose remains in the composition. Stopping removes that cache. Muting removes and suppresses the cache while playback continues; unmuting permits the next clip frame to recreate it as the newest source.
...
The compositor is infrastructure rather than an artist-started performance Routine. Pressing SuperCollider's command-period shortcut stops ordinary processes and synths. Bmc also stops its clip players and removes their caches, then starts a fresh compositor Routine after the clock is cleared. Existing motion-source routes, avatar mappings, and live camera caches are retained, so camera control continues and cannot remain hidden beneath a stopped clip cache. Use `Bmc.stopCompositor` or `Bmc.stop` when the compositor itself should remain stopped.
...
5. Apply the avatar's positional modifiers from oldest to newest. Each modifier may replace or add selected `Hips` translation axes using the current compositor time.
6. Fill any still-missing values from the reference pose.
7. Assign the destination avatar identity.
8. Publish one route-free completed frame to dependants.
9. Attach the VMC route at the avatar boundary.
10. Transmit exactly one frame for that avatar during the tick.
