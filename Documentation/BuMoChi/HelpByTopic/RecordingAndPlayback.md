...
| `OscRecorder` | writes successive limited-size `.scd` files while recording | may retain the complete recording in `recordedData` |

The active implementation is [Classes/Animation/OscRecorder.sc](../../../Classes/Animation/OscRecorder.sc). The older sc-dance `SessionData` and `OscFile` readers have been archived under `Deprecated/sc-dance-v1`; current BuMoChi does not depend on them. `Bmc.loadClipScd(path)` loads one complete `.scd` clip file. Importing an entire directory of numbered `OscRecorder` chunks as one logical Bmc Clip is not currently implemented.

This division is useful: use `OscRecorder` when uninterrupted, effectively unlimited capture to chunked files is the priority; use the Bmc clip methods when the recording must immediately participate in Bmc selection, composition, scene assignment, and playback.
...
# Implemented Scene Motion settings
...
1.  A *Motion name*: the name used inside a scene, such as `\motherEntrance`.
...
5.  Optional transport values: playback `rate`, `loop` state, and the `in` position in seconds.

The minimum useful Motion setting contains the clip and avatar names:
...
1.  Find the settings by their Motion name.
...
7.  Apply `rate`, `loop`, and `in`.
8.  Start playback.

This sequence is performed automatically by `Bmc.playSceneMotion`.

# Scenes

A BuMoChi Scene is the cross-system configuration used to animate one Godot scene. The currently implemented subset stores configured Motions, avatar OSC routes, and references to a Godot project and `.tscn` scene resource. It is configuration data, not recorded motion data: the complete immutable Clips remain in separate `.scd` files.

The proposed two-dictionary design is correct. Bmc stores both dictionaries inside one top-level scene data object:
...
Although a Motion name can equal its Clip name, storing the Clip reference explicitly is useful: a dramaturgical cue such as `\motherEntrance` can refer to the complete recording named `\mother_take_01`. The avatar name is required. The host is stored as well as the port so that the same scene format also works across computers. A bare port number is accepted as shorthand for localhost:
...
~scene = Bmc.saveScene(
    ...
    nil, // no shared decoder override
    \boyAndBirds,
...
``` example
Platform.userAppSupportDir/BmcScenes/duet_rehearsal.scd
```

The scene name becomes the filename. The `.scd` file contains readable sclang data and may be inspected or edited as text. A custom destination may be supplied as the fourth argument to `Bmc.saveScene`. Because loading an `.scd` file asks sclang to interpret its contents, load scene files only from sources you trust.

## Load and apply a scene

After recompiling or restarting SuperCollider, load by scene name:
...
`Bmc.loadScene` also accepts an absolute `.scd` path. `Bmc.applyScene` registers the configured avatars when needed and restores their OSC output routes. It does not load all clips; clips are loaded on demand.

## Play a configured Motion

Use the Scene-local Motion name, not necessarily the archive name:

``` supercollider
Bmc.playSceneMotion(\motherEntrance);

// Later, after the first playback has stopped:
Bmc.playSceneMotion(\ishidomaruReply);
```

`Bmc.play` and `Bmc.playClip` use the `\default` player unless a `playerName` is supplied. Named players can run simultaneously and are independently accessible through `Bmc.player(name)`. `Bmc.playSceneMotion` currently continues to use the default player.
