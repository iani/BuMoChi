# Sequence Editor: purpose and initial design
...
The first Sequence Editor should contain:

1. A Sequence list with **New**, **Rename**, **Save**, and **Clone** actions.
...
8. A visible playhead and current Sequence time.
9. A feedback/status line for validation and playback results.

A first layout may be:

```text
Sequence: performance_a       Godot project: boy_and_birds
...
- A Sequence must identify exactly one Godot project.
- Every Scene used by the Sequence must be associated with that project.
...
Sequence definitions should be stored as human-readable `.scd` data beneath:

```text
BuMoChiRecordings/Sequences
```

Each Sequence should use one named file. It stores its Godot-project reference, Scene references, ordered timeline cues, and timing—not Clip data, Preset contents, or Godot node trees.

## Proposed entry points

```supercollider
Bmc.sequenceEditor;
Bmc.sequenceEditor(\performanceA);
```

These methods and the Sequence data class are specifications only and are not yet implemented.
