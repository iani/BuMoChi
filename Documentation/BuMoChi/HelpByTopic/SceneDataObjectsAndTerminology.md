...
A *Sequence* adds temporal structure to this configuration: it is an ordered timeline of actions or cues associated with one Godot project. A Sequence operates in the project's default Godot Scene and may switch among other `.tscn` Scenes in that project. A Scene describes the reusable runtime configuration; a Sequence coordinates that configuration and Godot Scene selection over time.
