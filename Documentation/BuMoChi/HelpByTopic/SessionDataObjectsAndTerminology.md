...
A *Sequence* adds temporal structure to this configuration: it is an ordered timeline of actions or cues associated with one Godot project. A Sequence operates in the project's default Godot Scene and may switch among other `.tscn` Scenes in that project. A Session describes the reusable runtime configuration; a Sequence coordinates that configuration and Godot Scene selection over time.

# Session data format

Sessions use five principal data objects. *Source* is an additional role or interface: anything capable of supplying motion data can act as a source, but sources do not have to form a separate top-level session dictionary.
...
3.  *Motion*: A motion is a configured use of a motion source. Its source may be a clip (optionally through a preset), live stream, language-side Function, Synth-controlled generator, or another compatible producer. A motion can override preset values for one session without changing the stored preset. When evaluated, a motion supplies movement for all or part of a figure.
...
The `in` value is the position, in seconds, at which reading begins inside the clip. It replaces the less precise name `start`. A Sequence action uses a separate `at` value to specify when a motion begins on the Sequence timeline.
