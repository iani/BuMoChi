...
:PROPERTIES:
:CUSTOM_ID: distributed-sources-local-synthesis
:END:
...
:END:
/*The two Python command-line scripts described here can be started together with the one-command shell launcher described in [[file:HelpByTopic/Setup.md#quick-start-one-command-pipeline-launcher][Setup]].*/
...
/*The =OscGroupClient= application can optionally be started by the same one-command shell launcher described in [[file:HelpByTopic/Setup.md#quick-start-one-command-pipeline-launcher][Setup]].*/
...
:END:
See [[file:HelpByTopic/Setup.md#quick-start-one-command-pipeline-launcher][Setup]] for the easiest way to start the encoder.
