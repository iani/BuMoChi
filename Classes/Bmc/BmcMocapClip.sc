...
BmcMocapClip {
	...
	*new { |entries, metadata|
		^super.new.init(entries, metadata)
	}
	...
	duration {
		^if(entries.isEmpty) { 0.0 } { entries.last[0] }
	}

	avatar {
		^if(entries.isEmpty) { nil } { entries.first[1][2] }
	}

	source {
		^if(entries.isEmpty) { nil } { entries.first[1][3] }
	}

	asArray {
		^entries.collect { |entry| [entry[0], entry[1].copy] }
	}
}
