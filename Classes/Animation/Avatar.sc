...
// 3d argument of the ['/bunraku' ... ] message array. Example:
// An avatar named \girl1 would send the message like this:
// ['/bunraku', <timestamp>, 'girl1', 'hip' ...]
// - Handle forwarding of data received live from Bunraku MOCAP
...
Avatar : NamedInstance {
	...
	var <oscAddress = '/bunraku/avatar/frame'; // address for remote respnse

	*localGodotAddr {
		^localGodotAddr ?? { localGodotAddr = NetAddr("127.0.0.1", 22245); }
	}
	*sessionGui { AvatarAssets.sessionGui }

	*allSessionNames { // list sessions from all subclasses
		^AvatarAssets.allSessionNames;
	}
	addLocalGodot {
	...
	enableRemote { | argOscAddress = '/bunraku/avatar/frame' |
	...
	*reset { this.default.reset }
	...
	addReceiver { | netAddr | // asSymbol: guarante matching
		...
		// '/bunraku/avatar/frame' osc message with '/r' in order to avoid
	...
	publishValueArray { | publishMsg, argMsg |
		// get numeric values from raw bunraku message
